...
import { buildIndex, deviceColor, matchBook } from "../utils";
...
const MIN_GRID_WIDTH =
	MIN_COLS * CARD_MIN_WIDTH + (MIN_COLS - 1) * GAP + PADDING * 2;
...
export function BookGrid({
	...
	const cols = Math.min(
		...
		Math.max(
			MIN_COLS,
			Math.floor((effectiveWidth - PADDING * 2 + GAP) / (CARD_MIN_WIDTH + GAP)),
		),
	);
	...
	const _selectionCount = selectedBookIds.size;
	const focusedBook =
		books.find((b) => selectedBookIds.has(b.ID as number)) ?? null;
						...
						{bookRows[virtualRow.index].map((book, colIdx) => {
							...
							return (
								<CoverCard
									...
									index={globalIdx}
									isSelected={
										selectedBookIds.has(book.ID as number) ||
