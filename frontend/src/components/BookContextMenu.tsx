...
import type { Book, DeviceInfo } from "../types";
...
export function BookContextMenu({
	...
	const selectedBooks = visibleBooks.filter((b) =>
		selectedBookIds.has(b.ID as number),
	);
	const allRead =
		selectedBooks.length > 0 && selectedBooks.every((b) => b.IsRead);
			...
			<Menu>
				...
				{selectionCount === 1 &&
					...
					(formats.length === 1 ? (
						<MenuItem
							...
							icon="document-open"
							onClick={() => {
								onOpenBook?.(focusedBook?.ID as number, formats[0].Format);
								onClose();
							}}
						/>
					) : (
						<MenuItem text="Open" icon="document-open">
							{formats.map((f) => (
								<MenuItem
									...
									onClick={() => {
										onOpenBook?.(focusedBook?.ID as number, f.Format);
										onClose();
									}}
								/>
							))}
				...
				{selectionCount === 1 && (
					<MenuItem
						...
						icon="cloud-download"
						onClick={() => {
							if (focusedBook) onFetchMetadata?.(focusedBook);
							onClose();
						}}
					/>
				)}
