...
export function DevicePanel({ file, books, width, onWidthChange }: Props) {
	...
	return (
		<div
			ref={panelRef}
			className="shrink-0 h-full border-l border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex flex-col overflow-y-auto relative"
			style={{ width }}
		>
			<div
				aria-hidden="true"
			...
			<div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
				<dl className="flex flex-col gap-1.5 text-xs">
					...
					<div className="flex gap-2">
						...
						<dd className="text-zinc-600 dark:text-zinc-400 min-w-0 break-all">
							{file.Path}
						</dd>
					</div>
				</dl>
				...
				<>
					...
					{book.Tags?.length > 0 && (
						<div className="flex flex-wrap gap-1 px-4 py-3 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
							{book.Tags.map((t) => (
								<span
									key={t}
									className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-300 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300"
								>
									{t}
								</span>
							))}
					...
					{book.Description && (
						<div className="px-4 py-3 flex-1 overflow-y-auto">
							<div
								className="prose prose-invert prose-sm max-w-none text-zinc-600 dark:text-zinc-400 text-[14px] leading-tight [&_p]:my-0.5 [&_p]:leading-tight"
